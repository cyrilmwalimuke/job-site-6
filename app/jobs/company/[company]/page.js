...
export async function generateMetadata({ params }) {
  ...
  const company = formatTitle(params.company);
  const now = new Date();
const options = { year: 'numeric', month: 'long' };
const formattedDate = now.toLocaleDateString('en-US', options);
 
  return {
    title:` Jobs at ${company} - Apply Now`,
    description:  `Browse the latest job listings from  ${company}.`,
  ...
  openGraph: {
    ...
    description: `Discover new opportunities from ${company}.`,
    url:`${baseUrl}/jobs/company/${params.title}`,
    siteName: 'JobsKe',
    images: [
      {
  ...
  twitter: {
    ...
    description: `Top jobs from ${company}.`,
    site: '@jobs_ke',
    creator: '@jobs_ke',
...
const editedCompany = title.replace(/-/g, ' ')
  ...
  try {
    const res = await fetch(`${baseUrl}/api/find-by-company/${editedCompany}`, {
      cache: 'no-store',
    });
...
export default async function JobsPage({ params }) {
      ...
      const jobs= await getJobBySlug(company);
  ...
  Top Job Listings from <span className="capitalize">{company.replace(/-/g, ' ')}</span>
</h1>
              <p className="mt-2 text-gray-600 break-words overflow-hidden text-ellipsis">Browse the latest job listings from <span className="capitalize">{company.replace(/-/g, ' ')}</span></p>
            </div>
